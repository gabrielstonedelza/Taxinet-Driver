...
class _SendFromWalletToPassengerState extends State<SendFromWalletToPassenger> {
  ...
  Future<void> getSendersWallet()async {
    final profileLink = "https://taxinetghana.xyz/get_wallet_by_user/${userController.driverProfileId}/";
    var link = Uri.parse(profileLink);
    http.Response response = await http.get(link, headers: {
      "Content-Type": "application/x-www-form-urlencoded",
    });
  ...
  processUpdateSendersWallet()async{
    final depositUrl = "https://taxinetghana.xyz/user_update_wallet/${userController.driverProfileId}/";
    final myLink = Uri.parse(depositUrl);
